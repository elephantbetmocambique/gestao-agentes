...
const Auth = (() => {
  ...
  function login(username, password) {
    ...
    const session = { username: user.username, nome: user.nome, role: user.role };
  ...
  function can(permission) {
    const s = getSession();
    if (!s) return false;
    return !!(PERMISSIONS[s.role] && PERMISSIONS[s.role][permission]);
  }
  ...
  function showAppScreen() {
    ...
    document.getElementById("userRole").textContent = s.role === "admin" ? "Administrador" : "Assistente";
  ...
  function handleLogin() {
    ...
    if (!u || !p) { document.getElementById("loginError").textContent = "Preencha utilizador e senha."; return; }
    if (login(u, p)) {
      showAppScreen();
      App.init();
    } else {
  ...
  return { init, login, logout, getSession, can, isAdmin, handleLogin };
})();
...
const App = (() => {
  ...
  function checkConfig() {
    if (!CONFIG.APPS_SCRIPT_URL || CONFIG.APPS_SCRIPT_URL === "COLE_AQUI_O_URL_DO_APPS_SCRIPT") {
      document.getElementById("agentTableBody").innerHTML = `
        <tr><td colspan="8" class="empty-row" style="color:#b91c1c;line-height:1.8">
          <strong>Configuração incompleta</strong><br>
          Abra <code>js/config.js</code> e cole o URL do Apps Script.
        </td></tr>`;
      return false;
    }
    return true;
  ...
  // ---------- aplicar visibilidade por role ----------
  function applyRoleUI() {
    const isAdmin = Auth.isAdmin();
    // botões que só admins vêem
    document.querySelectorAll(".admin-only").forEach(el => {
      el.style.display = isAdmin ? "" : "none";
    });
    // mensagem de modo leitura
  ...
  function openEdit(id) {
    if (!Auth.can("canEdit") && !(!id && Auth.can("canCreate"))) {
      showToast("Sem permissão para esta acção.", "warning"); return;
    }
  ...
  async function doDelete() {
    ...
    try {
      ...
      closeConfirm(); showToast("Agente eliminado.", "warning"); render();
    } catch (e) {
    ...
    try {
      ...
      document.getElementById("agentTableBody").innerHTML = `
        <tr><td colspan="8" class="empty-row" style="color:#b91c1c;line-height:1.8">
          ...
          <small>Verifique o URL do Apps Script em js/config.js</small>
        </td></tr>`;
    } finally { setLoading(false); }
  }
...
const Importer = (() => {
  ...
  function parseRow(r) {
    ...
    if (!d.status || !["ativo","inativo","manut"].includes(d.status)) d.status = "ativo";
