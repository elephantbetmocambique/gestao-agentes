...
const App = (() => {
  ...
  function hasActiveFilters() {
    ...
    return !!(f.q || f.status || f.tipo || f.genero || f.supervisor);
  }
  ...
  function renderMetrics() {
    ...
    // Métricas baseadas nos agentes filtrados
    document.getElementById("metricTotal").textContent   = filtered.length;
    document.getElementById("metricAtivo").textContent   = filtered.filter(a => a.status === "ativo").length;
    document.getElementById("metricInativo").textContent = filtered.filter(a => a.status === "inativo").length;
    document.getElementById("metricManut").textContent   = filtered.filter(a => a.status === "manut").length;

    // Label dinâmico no card Total
    const labelEl  = document.getElementById("metricTotalLabel");
    const tagEl    = document.getElementById("filterActiveTag");
    if (labelEl)  labelEl.textContent   = isFiltered ? filtered.length + " de " + all.length : "Total de agentes";
    if (tagEl)    tagEl.style.display   = isFiltered ? "inline-flex" : "none";

    // Actualiza contadores secundários de cada card com total global entre parênteses
    const cards = [
      { id: "metricAtivoSub",   status: "ativo"   },
      { id: "metricInativoSub", status: "inativo" },
      { id: "metricManutSub",   status: "manut"   },
    ];
    cards.forEach(({ id, status }) => {
      const el = document.getElementById(id);
      if (!el) return;
      if (isFiltered) {
        const totalStatus = all.filter(a => a.status === status).length;
        el.textContent = "de " + totalStatus + " total";
  ...
  function renderFilterBar() {
    ...
    const all       = state.agents;
    const isFiltered = hasActiveFilters();
    const bar        = document.getElementById("filterClearBar");
    const msg        = document.getElementById("filterClearMsg");
    if (!bar) return;
    ...
    if (msg && isFiltered) {
      ...
      if (f.status)     parts.push({ ativo:"Activo", inativo:"Inactivo", manut:"Manutenção" }[f.status] || f.status);
      if (f.tipo)       parts.push(f.tipo);
      if (f.genero)     parts.push(f.genero === "M" ? "Masculino" : "Feminino");
      if (f.supervisor) parts.push(f.supervisor);
      msg.textContent = \`\${filtered.length} resultado\${filtered.length !== 1 ? "s" : ""} para: \${parts.join(" · ")}\`;
    }
  }

  function clearFilters() {
    state.filters = { q: "", status: "", tipo: "", genero: "", supervisor: "" };
    // Limpa os inputs visuais
    const ids = ["filterSearch","filterStatus","filterTipo","filterGenero","filterSup"];
    ids.forEach(id => {
      const el = document.getElementById(id);
      if (el) el.value = "";
    });
  ...
  return {
    init, reload, render, exportCSV, switchView, clearFilters,
